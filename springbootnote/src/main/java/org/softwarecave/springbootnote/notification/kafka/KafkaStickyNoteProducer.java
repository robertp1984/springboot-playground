...
@Slf4j
@Component
@ConditionalOnBooleanProperty(prefix = "app.kafka", name = "enabled", havingValue = true)
public class KafkaStickyNoteProducer {
    ...
    public KafkaStickyNoteProducer(JsonMapper jsonMapper,
                                   @Value("${app.kafka.bootstrap-servers}") String bootstrapServers,
                                   @Value("${app.kafka.stickynote-topic}") String noteTopic) {
    ...
    public void sendToKafka(StickyNote returnValue) {
        StickyNoteDTO dto = stickyNoteConverter.convertToDTO(returnValue);
        Long key = dto.getId();
        String value = jsonMapper.writeValueAsString(dto);
        var producerRecord = new ProducerRecord<>(noteTopic, key, value);
